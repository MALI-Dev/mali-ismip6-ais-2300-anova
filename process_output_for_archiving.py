"""
Process output dataset for archiving a cleaned subset of data.
"""
...
def main():
    parser = argparse.ArgumentParser(
                        description='process MALI outputs for archiving')
    parser.add_argument("-i", "--input_path", dest="input_path",
                        required=True,
                        help="path to MALI output directory to process")
...
def clean_xtime(ds):
    ...
    for t in range(nTime):
        xtime2_array[t] = xtime.values[t,:].tobytes().decode()  # convert to str
    xtime2 = xr.DataArray(xtime2_array, dims = ['Time'])  # convert back to char array
...
def process_state_files(input_path):
...
def time_avg_flux_vars(input_path):
    """
    input_file: path to MALI simulation flux files that have the all time levels
    output_file: file with time-averaged fluxes
    """
    print("Starting time averaging of flux variables")
    ds_in = xr.open_mfdataset(os.path.join(input_path, 'output_flux*.nc'),
                              combine='nested', concat_dim='Time',
                              decode_times=False, decode_cf=False)
    ...
    xtime = []
    for i in range(time):
        xtime.append(xtimeIn[i].tobytes().decode('utf-8').strip().strip('\x00'))
    #print(xtime)
    deltat = ds_in['deltat'][:]
    daysSinceStart = ds_in['daysSinceStart'].values
    cellMask = ds_in['cellMask'][:,:]
    sfcMassBal = ds_in['sfcMassBalApplied'][:, :]
    floatingBasalMassBalApplied = ds_in['floatingBasalMassBalApplied'][:, :]
    groundedBasalMassBalApplied = ds_in['groundedBasalMassBalApplied'][:, :]
    dHdt = ds_in['dHdt'][:,:] / (3600.0 * 24.0 * 365.0) # convert units to m/s
    glFlux = ds_in['fluxAcrossGroundingLineOnCells'][:, :]
    calvingThickness = ds_in['calvingThickness']
    faceMeltingThickness = ds_in['faceMeltingThickness']
    #calvingFlux = ds_in['calvingFlux'][:, :]
    #faceMeltAndCalvingFlux = ds_in['faceMeltAndCalvingFlux'][:, :]
    ...
    maxIceMask = np.zeros((len(years), nCells), dtype=int) * np.nan
    ...
    for j in range(len(years)):
        ...
        for i in range(time):
            ...
            if decYears[i] > years[j] and decYears[i] <= years[j]+1.0:
                ...
                sumYearCF = sumYearCF + calvingThickness[i,:] * deltat[i]
                sumYearFM = sumYearFM + faceMeltingThickness[i,:] * deltat[i]
